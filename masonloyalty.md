### MASON LOYALTY - MD file

1.  **Onboarding:** Correctly identifying `POST /api/masons` as the registration route that includes the **transactional joining bonus**.
2.  **Earning Points:** Detailing the *full* logic of the `PATCH /api/bag-lifts/:id` approval, which now includes `tsoAuth` protection and the atomic calculation for **Extra Bonuses** and **Referral Bonuses**.
3.  **TSO Auth:** Noting which routes are protected by the `tsoAuth` middleware, such as the points ledger.
4.  **Admin Routes:** Expanding the final section to include other critical TSO/Admin functions found in the files, like **manual point adjustments**, dealer creation, and viewing TSO-specific data.
...
| **6. TSO Bag Lift Approval** | `PATCH /api/bag-lifts/:id` | `bagLifts`, `pointsLedger`, `masonPcSide` | **(Protected by `tsoAuth`)<br>Atomic Transaction (Credit):**<br>1. Updates `bagLifts.status` from `'pending'` to **'approved'**.<br>2. Inserts a **POSITIVE** record into `pointsLedger` for the main `pointsCredited`.<br>3. Atomically updates `masonPcSide.pointsBalance` and `bagsLifted`.<br>4. **Calculates & applies Extra Bonus** (e.g., for crossing 250 bags) and inserts a *separate* ledger entry if \> 0.<br>5. **Checks & applies Referral Bonus** (if threshold is met) to the *referrer's* account, inserting a *third* ledger entry. |
| **7. TSO Rejection/Unwind** | `PATCH /api/bag-lifts/:id` | `bagLifts`, `pointsLedger`, `masonPcSide` | **(Protected by `tsoAuth`)<br>Atomic Transaction (Debit/Unwind):**<br>1. If reversing a previous approval (`'approved'` to `'rejected'`): inserts a **NEGATIVE** debit/adjustment record into `pointsLedger`.<br>2. Reduces `masonPcSide.pointsBalance` and `bagsLifted` to unwind the initial credit. |
...
| **13. Fulfillment Tracking** | `PATCH /api/rewards-redemption/:id` | `rewardRedemptions` | **(Protected by `tsoAuth`)**<br>TSO/Admin tool. Used to manually update the order's status through its lifecycle (e.g., `'placed'` → `'approved'` → `'shipped'` → `'delivered'`). |
